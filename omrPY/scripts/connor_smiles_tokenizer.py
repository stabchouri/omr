...
def get_vocab(data, explicit=True):
    '''Get the vocabulary from a dataframe with "prod_smiles" field'''
    ...
    num_tokens = 0
    N = 0
    for smiles in tqdm(data['prod_smiles']):
        try:
            if explicit:
...
if __name__ == '__main__':
    ...
    from retronn.data.get_data import get_data_df, split_data_df
    data = get_data_df('data/data_processed.csv')
    split_data_df(data)

    print('Getting vocab from training set only, using explicit tokens')
    vocab_counts = get_vocab(data.loc[data['dataset'] == 'train'])
