...
BASE_DIR ='/scratch2/sophiat/chem-ie-TJS_omrPY/'

image_directory = BASE_DIR + 'omrPY/im_smiles/src/model/data/train.img'
feed_directory = BASE_DIR + 'omrPY/im_smiles/src/model/data/train_filter.lst'
smiles_tokenized = BASE_DIR + 'omrPY/im_smiles/src/model/data/smiles.120k'
vocab_directory = BASE_DIR + 'omrPY/data/smiles_vocab.txt'
max_length = 100
enc_hidden_size = 256
dec_hidden_size = 256
enc_layers = 1
...
def train(inp_tensor, target_tensor, convolution, encoder, decoder, cnn_optimizer, encoder_optimizer,\
	...
	print('ok2')
	#depends on you version of pytorch may either use loss.data()
	return loss.data / target_tensor.size(1), top_p
...
def trainIters(convolution, encoder, decoder, epochs, print_every=1000, plot_every=100, learning_rate=0.01, teacher_forcing_ratio=0.3, gpu=False):
	...
	for iters in range(1, epochs + 1):
		dropped = 0
		s_batch = DataGenerator(feed_directory, smiles_tokenized, vocab_directory)
...
if __name__ == '__main__':
	...
	if gpu:
		...
		trainIters(conv, enc, dec, 1, print_every=100, plot_every=100, teacher_forcing_ratio=1.0, gpu=True)
	else:
		conv = CNNModel()
	...
	torch.save(enc.state_dict(),BASE_DIR + 'omrPY/im_smiles/src/model/model_enc')
	torch.save(dec.state_dict(),BASE_DIR + 'omrPY/im_smiles/src/model/model_dec')
